...
void disp(int n,int qt){
    ...
    for(i=0;i<n;i++){
        alltime+=a[i].btime;
        //cout<<"start is "<<a[i].pname<<" to "<<ttime<< "total time " << alltime<< "\n";
    }
    alltime+=a[0].atime;
    ...
    cout<<"P.Name \tArrival Time\t\tBust Time\t\tWaiting Time\n";
...
int main()
{
    ...
    for(int i=0;i<n;i++){
        a[i].pname =(i+48);
        cout<<"Enter arrival time for P"<<a[i].pname<<" : ";
        cin>>a[i].atime;
        cout<<"Enter buast time for P"<<a[i].pname<<" : ";
        cin>>a[i].btime;
